...
- 设置页支持三种 `x-statsig-id` 来源：**local（默认）** 在进程内用纯 Go CSS 动画指纹算法实时生成，无需浏览器、JS 或远程签名服务；**manual** 直接使用管理员写入的固定值，不自动失效、刷新或替换；**url** 会先使用同一账号、出口节点、User-Agent 与 Cookie 访问 `https://grok.com/index`，读取 `grok-site-verification` meta，再把 method/path/metaContent 发给自建签名服务。升级时会将历史第三方默认签名地址与空/占位 `127.0.0.1:8788` 自动迁移为 local。URL 签名按 method/path 缓存；Code 7 会立即强制刷新；local 模式每次请求重新签名。绝不发送随机占位签名。
