...
### 3.3 [高→已缓解] 默认依赖第三方 Statsig 签名服务

**位置：**

- `internal/infra/config/config.go`：`StatsigModeLocal`（默认）、历史 `DefaultStatsigSignerURL`
- `internal/infra/provider/web/statsig.go` + `internal/infra/provider/web/statsig/`（纯 Go 签名）

**机制（当前默认）：**

1. **local 模式（默认）**：进程内用 CSS 动画指纹算法生成 `x-statsig-id`，不访问远程签名服务、不抓取首页 meta。
2. **url 模式（可选）**：账号 Cookie 拉 index → meta → 自建签名服务；禁止内置第三方默认 URL。
3. **manual 模式**：管理员固定写入 `x-statsig-id`。
...
- 默认 `StatsigModeLocal`；`NormalizeLegacyStatsig` 将空 / 第三方 / `127.0.0.1:8788` 占位迁移为 local
- 拒绝 `https://grok.wodf.de/sign` 作为生产 url 模式端点
- `signerurl.Validate` 限制 url 模式签名 URL 形态
- 签名客户端 `CheckRedirect = ErrUseLastResponse`；响应体上限与 70 字节格式校验
...
| 2026-07-16 | Statsig 默认改为进程内纯 Go `local` 模式；遗留第三方/占位 URL 自动迁移；url/manual 仍可选 |
